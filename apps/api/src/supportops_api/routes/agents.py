...
from supportops_core.agent_services import get_usable_agent, list_available_agents
...
async def read_available_agents(
    ...
    agents = await list_available_agents(session, identity=identity)
    return AgentCatalogResponse(items=[_safe_profile(agent) for agent in agents])
