...
- [x] 3A.6 将模型端点改为仅使用 `is_enabled`，新增连接下多模型稳定实体、逐模型不可变版本关联和数据库迁移
- [x] 3A.7 实现供应商预设、显式模型列表发现、连接与模型集合保存，并保持回车本地添加和远端发现语义分离
- [x] 3A.8 实现逐模型真实流式连通性测试、阶段耗时、持久化测试历史、测试过期判断和端点启用约束
...
- [x] 5.10 提供模型预设、发现、连接配置保存、逐模型测试/历史、启用与停用 API，并同步 OpenAPI 字段说明
...
- [x] 7.10 将模型管理改造成供应商预设和连接配置两步弹窗，支持手动模型添加、远端多选、逐模型名称/JSON 扩展对象和保存/保存并使用
- [x] 7.11 实现逐模型连通性测试弹窗、真实阶段进度、耗时和脱敏诊断展示，并在列表与编辑页展示测试汇总
...
- [x] 8.5 使用管理员和员工本地令牌完成“创建—编辑—发布—授权—选择—对话”的浏览器端到端验证
...
- [x] 9.5 添加真实 PostgreSQL 历史数据回填与约束测试，确认现有消息、Run 和事件不丢失
...
- [x] 9.11 添加规范 Agent 路由、旧路由不可用、动态品牌、按 Agent 会话过滤和刷新无闪烁浏览器测试
- [x] 9.12 添加多模型保存、显式发现、逐模型测试状态/过期、启用约束、密钥脱敏和前端构建回归测试
