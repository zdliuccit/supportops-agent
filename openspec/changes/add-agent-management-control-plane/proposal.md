...
- 新增独立模型端点管理，支持 OpenAI 官方和 OpenAI-compatible 中转站；一个端点表示一套供应商连接，可以管理多个远端模型，每个模型只编辑模型名称与默认 `{}` 的 JSON 扩展对象，并独立记录测试状态和不可变版本。
- 模型端点创建采用“供应商预设—连接与模型”两步交互；手动回车添加模型只修改本地选择，显式“获取模型列表”才调用中转站发现接口。
- 模型端点不使用草稿或发布状态，只保留是否启用；“保存”写入并停用，“保存并使用”仅在当前连接下所有选中模型测试通过后启用。
